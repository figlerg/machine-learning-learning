...
# C = 2
# Normalize data
# X = stats.zscore(X); # shouldn't be necessary, we already did thatX = X_normalized

X = X_normalized
y = np.array(list((Y[:,1])), dtype=float).reshape(-1,1)
print(y)
...
y_est = y_test_est.data.numpy().reshape(-1,1); # squeeze was necessary, but now changed input
